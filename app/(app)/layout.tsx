...
import { getRealRole, getSessionPlayerId, resolveRole } from "@/lib/auth/access";
...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  const [role, realRole, playerId] = await Promise.all([resolveRole(), getRealRole(), getSessionPlayerId()]);
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <div className="flex flex-1 flex-col md:flex-row">
        ...
        <main className="flex-1 p-4 md:p-8 max-w-6xl w-full mx-auto">
          <RoleProvider role={role} playerId={playerId}>{children}</RoleProvider>
        </main>
      </div>
    </div>
  );
